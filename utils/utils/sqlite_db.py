...
class SQLiteDB:
    def __init__(self, db_path: Path):
        ...
        db_path = get_absolute_path(db_path, self.path)
        """
        初始化数据库连接
...
if __name__ == "__main__":
    ...
    # 初始化数据库操作类
    with SQLiteDB(Path(BASE_DIR / "db" / "database.db")) as db:
        # 插入示例
        ...
        # 查询示例
        query_sql = "SELECT * FROM file_records WHERE filesize > ?"
        results = db.query(query_sql, (1000,))
        print("查询结果:", results)